...
def model_fn(features,labels,mode,params):
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    tf.summary.scalar('p0loss', p0loss)
    tf.summary.scalar('vloss', vloss)
    tf.summary.scalar('p0_and_v_loss',p0loss+ vloss)
    tf.summary.scalar('vtimeloss', vtimeloss)
    printed_model_yet = True
    ...
    if os.path.exists(savepath):
      ...
      if swa_sub_epoch_scale is not None:
        ...
        if saved_to != os.path.join(savepathtmp,"non_swa_saved_model"):
          os.replace(saved_to, os.path.join(savepathtmp,"non_swa_saved_model"))
        ...
        if saved_to != os.path.join(savepathtmp,"saved_model"):
          os.replace(saved_to, os.path.join(savepathtmp,"saved_model"))
      ...
      time.sleep(1)
      shutil.move(savepathtmp,savepath)
