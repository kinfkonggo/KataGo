...
class Model:
  ...
  def build_model(self,config,placeholders):
    ...
    transformed_global_inputs = global_inputs
...
class Target_vars:
  def __init__(self,model,for_optimization,placeholders):
    ...
    variance_time_prediction = tf.math.softplus(miscvalues_output[:,3]) * 20.0
    ...
    self.variance_time_loss_unreduced = 200.00000 * huber_loss(1.0/(self.variance_time_target+10),1.0/(variance_time_prediction+10), delta = 0.03)
    ...
    if for_optimization:
      ...
      self.opt_loss = (
        ...
        self.value_loss +
        self.td_value_loss +
        #0.3*self.scoremean_loss +
        #0.3*self.lead_loss +
        #0.2*self.variance_time_loss +
        #0.3*self.scorebelief_pdf_loss +
        #0.3*self.scorebelief_cdf_loss +
        #0.3*self.ownership_loss +
        #0.3*self.scoring_loss +
        #0.3*self.futurepos_loss +
        #0.0*self.seki_loss +
        #0.3*self.scorestdev_reg_loss +
        self.reg_loss +
        self.scale_reg_loss
      )
