...
NodeStats::NodeStats()
  :visits(0),winValueSum(0.0),noResultValueSum(0.0),scoreMeanSum(0.0),scoreMeanSqSum(0.0),leadSum(0.0),utilitySum(0.0),utilitySqSum(0.0),weightSum(0.0), weightSqSum(0.0),varTimeSum(0.0)
{}
NodeStats::~NodeStats()
{}
  ...
  varTimeBuf()
{
  ...
  varTimeBuf.resize(NNPos::MAX_NN_POLICY_SIZE);
...
void Search::setNbest(double n) {
  searchParams.nbest=n;
}
...
bool Search::isLegalTolerant(Loc moveLoc, Player movePla) const {
  bool isStrict = false;
  return rootBoard.isLegal(moveLoc, rootPla, isStrict);
}
...
static double cpuctExploration(int64_t totalChildVisits, const SearchParams& searchParams) {
  if(searchParams.cpuctExploration>0)
    ...
    return -searchParams.cpuctExploration*
    pow((totalChildVisits + searchParams.cpuctExplorationBase) / searchParams.cpuctExplorationBase, searchParams.cpuctExplorationLog);
}

double Search::getExploreSelectionValue(
  double nnPolicyProb, int64_t totalChildVisits, int64_t childVisits,double childExpectFinishMovenum,
  ...
  double movenumComponent = searchParams.finishMovenumFactor *valueComponent * 10.0 / (10.0 + childExpectFinishMovenum);

  return exploreComponent + valueComponent+ movenumComponent;
}
...
double Search::getExploreSelectionValue(
  ...
  double expectFinishMovenumSum = child->stats.varTimeSum;
  double weightSum = child->stats.weightSum;
  int32_t childVirtualLosses = child->virtualLosses;
  child->statsLock.clear(std::memory_order_release);
  double expectFinishMovenum = expectFinishMovenumSum / weightSum + 1.0;
...
double Search::getNewExploreSelectionValue(const SearchNode& parent, float nnPolicyProb, int64_t totalChildVisits, double fpuValue, SearchThread* thread) const {
  ...
  while (parent.statsLock.test_and_set(std::memory_order_acquire));
  double expectFinishMovenum = parent.stats.varTimeSum/ parent.stats.weightSum;
  parent.statsLock.clear(std::memory_order_release);
  return getExploreSelectionValue(nnPolicyProb,totalChildVisits,childVisits,expectFinishMovenum, childUtility,parent.nextPla);
}
  ...
  bool isRoot) const
{
  ...
  for(int i = 0; i<numChildren; i++) {
    ...
    if (isRoot && searchParams.nbest > 1&& totalChildVisits>1000)
    {
      while (child->statsLock.test_and_set(std::memory_order_acquire));
      double childVisits = child->stats.visits;
      child->statsLock.clear(std::memory_order_release);
      double decrease = searchParams.nbest*childVisits/ totalChildVisits-1;
      if (decrease > 0)selectionValue -= 5*decrease;
...
void Search::recomputeNodeStats(SearchNode& node, SearchThread& thread, int numVisitsToAdd, int32_t virtualLossesToSubtract, bool isRoot) {
  ...
  vector<double>& varTimeSums = thread.varTimeBuf;
  ...
  for(int i = 0; i<numChildren; i++) {
    ...
    double varTimeSum = child->stats.varTimeSum;
    child->statsLock.clear(std::memory_order_release);
    ...
    varTimeSums[numGoodChildren] = varTimeSum;
    totalChildVisits += childVisits;
  ...
  for(int i = 0; i<numGoodChildren; i++) {
    ...
    varTimeSum += weightScaling * varTimeSums[i];
  }
  varTimeSum += weightSum;//因为往前推了一手，所以加1
  //Also add in the direct evaluation of this node.
  {
    ...
    double vtime = (double)node.nnOutput->varTimeLeft;
  ...
  node.stats.varTimeSum = varTimeSum;
...
void Search::addLeafValue(SearchNode& node, double winValue, double noResultValue, double scoreMean, double scoreMeanSq, double lead,double expectFinishMovenum, int32_t virtualLossesToSubtract, bool isTerminal) {
  double utility =
  ...
  node.stats.varTimeSum += expectFinishMovenum;
  node.virtualLosses -= virtualLossesToSubtract;
  node.statsLock.clear(std::memory_order_release);
}
...
void Search::initNodeNNOutput(
  ...
  nnInputParams.useVCF = searchParams.useVCF;
  ...
  bool certainWin = false;
  if (VCTcolor == opp)
  { 
    uint8_t res;
    uint16_t l;
    VCFsolver::run(thread.board, opp, res,l);
    if (res == 2)certainWin = true;
  }
  if (certainWin)node.isCertainLose = true;
  else
  {
...
void Search::addCurentNNOutputAsLeafValue(SearchNode& node, int32_t virtualLossesToSubtract) {
  ...
  if (node.isCertainLose)
  {
    node.nextPla;

    double winProb = (node.nextPla==C_BLACK)?0.0:1.0;
    double noResultProb = 0;
    double scoreMean = 0;
    double scoreMeanSq = 0;
    double lead = 0;
    double vtime = 0;
    addLeafValue(node, winProb, noResultProb, scoreMean, scoreMeanSq, lead, vtime, virtualLossesToSubtract, false);
  }
  else
  {
    double winProb = (double)node.nnOutput->whiteWinProb;
...
void Search::playoutDescend(
  ...
  if(!isRoot && thread.history.isGameFinished &&
    ...
    if(thread.history.isNoResult) {
      ...
      double lead = 0.0;
      addLeafValue(node, winValue, noResultValue, scoreMean, scoreMeanSq, lead,0, virtualLossesToSubtract,true);
      return;
    }
    else {
  ...
  if (node.isCertainLose)
  {
    double winValue = ScoreValue::whiteWinsOfWinner(node.nextPla, searchParams.drawEquivalentWinsForWhite);
    double noResultValue = 0.0;
    double scoreMean = 0;
    double scoreMeanSq = 0;
    double lead = scoreMean;
  ...
  if(bestChildIdx >= 0 && !thread.history.isLegal(thread.board,bestChildMoveLoc,thread.pla)) {
    ...
    if (thread.logStream != NULL)
    {
      ...
      Board::printBoard(cout, thread.board, Board::NULL_LOC, NULL);
      cout << Location::getX(bestChildMoveLoc, 15) << " " << Location::getY(bestChildMoveLoc, 15) << endl;
    }
