...
struct NodeStats {
  ...
  double weightSum;
  double weightSqSum;
  double varTimeSum;//预期终局步数，用utility加权了的
...
private:
  ...
  void addLeafValue(SearchNode& node, double winValue, double noResultValue, double scoreMean, double scoreMeanSq, double lead, double expectFinishMovenum, int32_t virtualLossesToSubtract, bool isTerminal);
  ...
  AnalysisData getAnalysisDataOfSingleChild(
    ...
    double parentScoreMean, double parentScoreStdev, double parentLead, double parentNoResultValue, int maxPVDepth
  ) const;
