  ...
  if (gameRand.nextBool(0.3))//黑棋乱撒
  {
    ...
    for (int y = 0; y < board.y_size; y++)
      for (int x = 0; x < board.x_size; x++)
      {
        ...
        if (gameRand.nextBool(fillRate)&&board.isLegal(loc, C_BLACK, false))hist.makeBoardMoveAssumeLegal(board,loc, C_BLACK,NULL);
      }
  }
  else //指定开局
  {
    const int numOpenings = 7;
    ...
    for (int y = 0; y < board.y_size; y++)
      ...
      {
        Loc loc = Location::getLoc(x, y, board.x_size);
        if (gameRand.nextBool(fillRate)&&board.isLegal(loc, color, false))hist.makeBoardMoveAssumeLegal(board,loc, color,NULL);
      }
  }
