...
int openingLoadTried = 0;
int openingLoadSucceed = 0;
struct Opening
{
...
bool hasLoadOpenings = false;
bool noOpeningsFile = false;
vector<Opening> openings;
static void loadOpenings()
{
  if (hasLoadOpenings || noOpeningsFile)return;
  noOpeningsFile = true;
#if RULE==RENJU
  ifstream ifs("openingsRenju.txt");
#else
  ifstream ifs("openingsGomoku.txt");
#endif
  if (!ifs.good())
  {
    return;
  ...
  {
    openings[i].prob /= probTotal;
  }
  hasLoadOpenings = true;
  noOpeningsFile = false;
  cout << "Successfully load " << openingsNum << " openings" << endl;
}
...
static void playGameRandomMove(Board& board, Rand& gameRand, Rules& rules)
{
  Player pla = board.movenum % 2 ? P_WHITE : P_BLACK;
  int xsize = board.x_size, ysize = board.y_size;
  if (board.movenum == 0)
  {
    int x = gameRand.nextUInt(xsize - 2) + 1, y = gameRand.nextUInt(ysize - 2) + 1;
    board.playMoveAssumeLegal(Location::getLoc(x, y, xsize), pla);
    return;
  }
  ...
  for (int x = 0; x < xsize; x++)for (int y = 0; y < ysize; y++)
  {
...
#if RULE==STANDARD
    if (board.colors[loc] != C_EMPTY || board.getMovePriorityAssumeLegal(pla, loc,false) == MP_FIVE)
#else
    if (board.colors[loc] != C_EMPTY || board.getMovePriorityAssumeLegal(pla, loc, true) == MP_FIVE)
#endif
    {
      prob[y*xsize + x] = 0;
      continue;
    ...
    for (int x2 = 0; x2 < xsize; x2++)for (int y2 = 0; y2 < ysize; y2++)
    {
      ...
      double dis = (x2 - x)*(x2 - x) + (y2 - y)*(y2 - y);
      dis = sqrt(dis);
      prob[y*xsize + x] += (dis < 5 ? 5 - dis : 0);
    }
  ...
  double randomDouble = gameRand.nextDouble();
  randomDouble *= totalProb;
  ...
  //unreachable
  //return Location::getLoc(0, 0, xsize);

}
static double getBoardValue(Search* botB, Search* botW, Board& board, const Rules& rules)
{
  Player pla = board.movenum % 2 ? P_WHITE : P_BLACK;
  ...
  if (pla == C_BLACK)return -value;
  else return value;
}
static bool playGameBalanceMove(Search* botB, Search* botW, Board& board, Rand& gameRand, const Rules& rules)
{
  openingLoadTried++;
  Player pla = board.movenum % 2 ? P_WHITE : P_BLACK;
  int xsize = board.x_size, ysize = board.y_size;
  ...
  for (int x = 0; x < xsize; x++)for (int y = 0; y < ysize; y++)
  {
    ...
    int xmin = std::max(x - 6, 0);
    int ymin = std::max(y - 6, 0);
    int xmax = std::min(x + 6, xsize - 1);
    int ymax = std::min(y + 6, ysize - 1);

    bool foundNearbyStone = false;
    if (board.movenum == 0)foundNearbyStone = true;
    for (int xt = xmin; xt <= xmax; xt++)
    {
      if (foundNearbyStone)break;
      for (int yt = ymin; yt <= ymax; yt++)
      {
        Loc loct = Location::getLoc(xt, yt, xsize);
        if (board.colors[loct] != C_EMPTY)
        {
    ...
    double value = getBoardValue(botB, botW, newBoard, rules);
    prob[y*xsize + x] = pow(1 - value * value, 4);
    if (prob[y*xsize + x] > maxProb)maxProb = prob[y*xsize + x];

  }
  if (gameRand.nextBool(1 - maxProb) && gameRand.nextBool(0.995))
  {
    //	cout << "opening failed " <<maxProb<< endl;
    return false;
  }
  ...
  for (int x = 0; x < xsize; x++)for (int y = 0; y < ysize; y++)
  {
    ...
    if (probSum >= randomDouble)
    {
      Player pla = board.movenum % 2 ? P_WHITE : P_BLACK;
      board.playMoveAssumeLegal(Location::getLoc(x, y, xsize), pla);
      //	cout << sqrt(1 - pow(prob[y*xsize + x], 0.25)) << endl;
      return true;
    }
  }
...
static bool tryGetInitializeGameUsingBalanceRandomOpening(
  Search* botB, Search* botW, Board& board,
  Rand& gameRand, Rules rules, int randomMoveNum) 
{
  Board newBoard(board.x_size, board.y_size);
  for (int i = 0; i < randomMoveNum; i++)playGameRandomMove(newBoard, gameRand, rules);
  double value1 = getBoardValue(botB, botW, newBoard, rules);
  if ((!gameRand.nextBool(0.5*value1 + 0.5)) && gameRand.nextBool(0.995))return false;

  Board tmpBoard(newBoard);
  Player pla = tmpBoard.movenum % 2 ? P_WHITE : P_BLACK;
  tmpBoard.playMoveAssumeLegal(Location::getLoc(0, 0, tmpBoard.x_size), pla);

  double value2 = getBoardValue(botB, botW, tmpBoard, rules);
  if ((!gameRand.nextBool(0.5*value2 + 0.5)) && gameRand.nextBool(0.995))return false;

  bool suc = playGameBalanceMove(botB, botW, newBoard, gameRand, rules);
  if (!suc)return false;
  board = newBoard;
  return true;

}
static void initializeGameUsingBalanceRandomOpening(
  Search* botB, Search* botW, Board& board, BoardHistory& hist, Player& pla,
  Rand& gameRand) 
{
  static const int maxRandomMoveNum = 17;
  
  static const double randomMoveNumProb[maxRandomMoveNum] = { 3,5,10,15,15,7,3,2,1,0,0,0,0,0,0,0,0 };//NoVC/VC0
  ...
  double randomMoveNumProbRandomDouble = gameRand.nextDouble()*randomMoveNumProbTotal;
  int randomMoveNum = 2 * maxRandomMoveNum;
  ...
  int tryTimes = 0;
  while (!tryGetInitializeGameUsingBalanceRandomOpening(
    botB, botW, board,
    gameRand, hist.rules, randomMoveNum))
  {
    tryTimes++;
    ...
    }//use another randomMoveNum
    */
    }
  }
  openingLoadSucceed++;
  //cout << "succeed=" << succeed << " tried=" << tried << endl;
  Rules rulesCopy = hist.rules;
  pla = board.movenum % 2 ? P_WHITE : P_BLACK;
  hist.clear(board, pla, rulesCopy);
...
void GameInitializer::createGameSharedUnsynchronized(
  ...
  if(posSample != NULL) {
    ...
    pla = P_BLACK;
    getRandomOpening(rand, board, pla);
    rules.komi = 0;
...
#if (defined CHANGE_FORBIDDEN_POLICY) && (RULE==RENJU)

  if (node->nextPla == C_BLACK)
  {
    const Board& board = toMoveBot->getRootBoard();
    for (int x = 0; x < board.x_size; x++)
      for (int y = 0; y < board.y_size; y++)
      {
        ...
        {
          locsBuf.push_back(loc);
          playSelectionValuesBuf.push_back(totalValue);
          maxValue = totalValue;
        }
      }
  }
...
static void recordTreePositionsRec(
  ...
  for(int i = 0; i<node->numChildren; i++) {
    ...
    histCopy.makeBoardMoveAssumeLegal(copy, child->prevMoveLoc, pla);
...
static Loc getGameInitializationMove(
  ...
  Rand& gameRand, double temperature,double& policyMultiplyLog
) {
  NNEvaluator* nnEval = (pla == P_BLACK ? botB : botW)->nnEvaluator;
  MiscNNInputParams nnInputParams;
  ...
  double totalplaySelectionValues = 0;
  ...
  for(int movePos = 0; movePos<policySize; movePos++) {
    ...
    totalplaySelectionValues += pow(policyProb, 1.0 / temperature);
  }
...
static void initializeGameUsingPolicy(
 ...
 // hist.makeBoardMoveAssumeLegal(board, Location::ofString("a1", Board::MAX_ARR_SIZE, Board::MAX_ARR_SIZE), hist.presumedNextMovePla);
...
FinishedGameData* Play::runGame(
  ...
  for(int i = 0; i<maxMovesPerGame; i++) {
    ...
    if (willDrawMidGame)
    {
      ...
      shouldDraw = true;
      int p = historicalMctsDrawValues.size()-1;
      for (int i = 0; i < 5; i++)
      {
        if (p < 0 || historicalMctsDrawValues[p] < 0.95)
        {
          shouldDraw = false;
          break;
        }
  ...
  if(recordFullData) {
    ...
    if(hist.isGameFinished &&( hist.isNoResult||hist.winner==C_EMPTY)) {
      ...
      if(!hist.isGameFinished) hist.endGamePla(C_EMPTY);
      double drawEquivalentWinsForWhite = 0.5;
      finalValueTargets.win = (float)ScoreValue::whiteWinsOfWinner(hist.winner, drawEquivalentWinsForWhite);
    ...
    std::fill(gameData->finalWhiteScoring, gameData->finalWhiteScoring + Board::MAX_ARR_SIZE, 0.0f);
...
void Play::maybeForkGame(
  ...
  NNResultBuf buf;
  double noResultUtilityForWhite = 0.5;
