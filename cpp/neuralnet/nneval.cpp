...
void NNEvaluator::evaluate(
  ...
  uint8_t oppvcfres;
  uint16_t oppvcfloc;
  if (nnInputParams.useVCF)
  {
    VCFsolver::run(board, nextPlayer, myvcfres, myvcfloc);//我自己能不能直接vcf
  ...
  else
  {
    ...
    oppvcfloc = Board::NULL_LOC;
  }


  //if (myvcfres == 3 || oppvcfres == 3)buf.vcfHitLimit = true;//hit limit
  ...
  if(!debugSkipNeuralNet) {
    ...
    if(inputsVersion == 7)
      NNInputs::fillRowV7(board, history, nextPlayer, nnInputParams, nnXLen, nnYLen, inputsUseNHWC, buf.rowSpatial, buf.rowGlobal, myvcfres,oppvcfres,myvcfloc);
    else
      ASSERT_UNREACHABLE;
  }
  ...
  if(hadResultWithoutOwnerMap) {
    ...
    //Normal case
      ...
      for (int i = 0; i < policySize; i++)
      {
        ...
        if (policy[i] <= 1e-30 && policy[i] >= 0)policy[i] = -1.0f;//将被排除掉的招法置为零
      }
    }
    ...
    if(modelVersion == 8) {
      ...
      double varTimeLeft;
      {
        ...
        double scoreMeanPreScaled = 0;
        double scoreStdevPreSoftplus = 0;
        double leadPreScaled = 0;
        ...
        varTimeLeft = softPlus(varTimeLeftPreSoftplus) * 20.0;
