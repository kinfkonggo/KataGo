...
Hash128 Board::getPosHashAfterMove(Loc loc, Player pla) const {
  ...
  bool wouldBeSuicide = isSuicide(loc,pla);
...
void Board::calculateAreaForPla(
  ...
  while(true) {
    ...
    for(int i = 0; i<numPlaHeads; i++) {
      ...
      if(vitalCountByPlaHead[plaHead] < 1) {
  ...
  for(int i = 0; i<numRegions; i++) {
    ...
    bool shouldMark = numInternalSpacesMax2[i] <= 0 && !bordersNonPassAlivePlaByHead[head] && atLeastOnePla; //internal space can be 2
